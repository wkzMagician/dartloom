...
- stable X25519 device identity generation through an injected secure-store
  contract;
- portable application-defined-scheme invitations with a ten-minute default
  lifetime;
...
delivery. The application receives only the verified pairing result and persists its
own Device endpoint.
